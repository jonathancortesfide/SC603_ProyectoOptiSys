...
import DatosDelExamen from './DatosDelExamen';
import { Stack } from '@mui/system';
import { AgregarExamen } from '../../requests/examenes/RequestsExamenes';

const steps = ['Examen', 'Aro', 'Proforma'];
...
const Examenes = () => {
  ...
  const agregarExamen = async () => { 
    ...
    let examen = await obtenerValoresDelExamen();
    AgregarExamen(examen).then((response) => {
      if(response.esCorrecto){
        console.log('Examen agregado correctamente');
      } else {
        alert("Ocurrio un error guardando el exámen, intente nuevamente!");
      }
    });
  };

  const obtenerValoresDelExamen = async () => {
    const examen = {
      NoExamen: 0, 
		  NoPaciente: 0, 
  ...
  return (
    <PageContainer>
      <Breadcrumb title="Examenes" description="Examenes" />
      <ParentCard title='Crear nuevo exámen'>
        <Box width="100%">
