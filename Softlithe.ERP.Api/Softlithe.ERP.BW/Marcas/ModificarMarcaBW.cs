...
{
    public class ModificarMarcaBW : IModificarMarcaBW
    {
        private readonly IModificarMarcaDA _modificarMarcaDA;
        private readonly IAgregarEventoBitacoraBW _agregarEventoBitacoraBW;
        private readonly IErrorLogger _logger;
        public ModificarMarcaBW(IModificarMarcaDA modificarMarcaDA, IAgregarEventoBitacoraBW agregarEventoBitacoraBW, IErrorLogger errorLogger)
        {
            _modificarMarcaDA = modificarMarcaDA;
            _agregarEventoBitacoraBW = agregarEventoBitacoraBW;
            _logger = errorLogger;
        }
        public async Task<ModeloValidacion> ModificarMarca(MarcaDto marcaDto)
        {
            try
            {
                int resultadoNo_marcaIncluida = await _modificarMarcaDA.ModificarMarca(marcaDto);
        ...
        private ModeloValidacion ConstruirRespuestaExitosa(int no_marcaIncluida, int errorBitacora)
        {
            ...
            {
                Mensaje = (no_marcaIncluida == 0 ? MensajeDeMarcaDto.MarcaNoGuadar : MensajeDeMarcaDto.MarcaModificadaCorrectamente) + (errorBitacora == 0 ? MensajesGeneralesDelSistemaDto.ErrorGuardarBitacora : string.Empty),
        ...
        private async Task<int> AgregarEventoBitacoraCorrecto(MarcaDto marcaDto, int no_marcaIncluida)
        {
            ...
            {
                descripcionDelEvento = no_marcaIncluida > 0 ? MensajeDeMarcaDto.MarcaModificadaCorrectamente + ". Descripción Marca: " + marcaDto.Descripcion : MensajeDeMarcaDto.MarcaNoGuadar + ". Descripción Marca: " + marcaDto.Descripcion,
                fechaDeRegistro = DateTime.Now,
                nombreDelMetodo = nameof(ModificarMarca),
                tabla = "Marca",
