...
public class ModificarUsuarioBW : IModificarUsuarioBW
{
    ...
    public async Task<ModeloValidacion> ModificarUsuario(ModificarUsuarioDto dto)
    {
        try
        {
            int resultado = await _modificarUsuarioDA.ModificarUsuario(dto);
    ...
    private ModeloValidacion ConstruirRespuestaExitosa(int resultadoRegistro, int errorBitacora)
    {
        ...
        {
            Mensaje = (resultadoRegistro > 0 ? MensajeDeUsuarioDto.UsuarioModificadoCorrectamente : MensajeDeUsuarioDto.UsuarioNoGuardado)
                      + (errorBitacora == 0 ? MensajesGeneralesDelSistemaDto.ErrorGuardarBitacora : string.Empty),
            EsCorrecto = resultadoRegistro > 0,
        };
    }

    private async Task<int> AgregarEventoBitacoraCorrecto(ModificarUsuarioDto dto, int resultadoRegistro)
    {
        ...
        return await _agregarEventoBitacoraBW.AgregarEventoBitacora(new BitacoraDto
        {
            descripcionDelEvento = resultadoRegistro > 0
                ? MensajeDeUsuarioDto.UsuarioModificadoCorrectamente + " id_usuario: " + dto.IdUsuario + " Nombre: " + nombre
                : MensajeDeUsuarioDto.UsuarioNoGuardado + " id_usuario: " + dto.IdUsuario + " Nombre: " + nombre,
            fechaDeRegistro = DateTime.Now,
            nombreDelMetodo = nameof(ModificarUsuario),
            tabla = "Usuario",
